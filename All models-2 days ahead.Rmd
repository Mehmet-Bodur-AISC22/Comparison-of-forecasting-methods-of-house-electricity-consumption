---
title: "all forecasting models 2 days"
...
Forecasting 2 days ahead. Models are BRNN with LSTM layer, SVR WITH (ANOVA RBF kernel), and SBC with Fuzzy C-mean.
...
6000 consecutive random rows of data is selected from the main data set(starting point is random)
...
sys = function(N,n){
    r = sample(1:(N-n), 1)
    seq(r, ((r + 6000) -1), 1)    #selecting 4 days consecutive random data set
}
sys_sample<-raw_data[sys(nrow(raw_data), 6000), ]

data <- data.matrix(sys_sample[1:6000, -1])
...
data2 = data2[, colSums(data2 != 0) > 0] #if there is any column with all zero values omit the column


#write.csv(sys_sample, 'data-tenthrun-2days.csv')
```

data preparation for our BRNN model
1500 assigned to training set and 1500 verification data
...
Data preparation for SVR and SBC models, also preparing real values of foretasted output  to evaluate our models later, ninth column is the output 

```{r}
data.train <- matrix(data2[1 : 3000, ],ncol = 9, byrow = FALSE)
...
data.tst <- matrix(data2[3001:6000, 1 : 8 ],ncol = 8, byrow = FALSE)
x <- data.train[, -ncol(data.train)]
y <- data.train[,9]

real.val.SVM <- matrix(data2[3001:6000, 9], ncol = 1)
real.val.SBC <- matrix(data2[3001:6000, 9], ncol = 1)
real.val.ANN <- matrix(data2[3001:6000, 9], ncol = 1)

range.data <- matrix(apply(data.train, 2, range), nrow = 2)
```

Building bidirectional RNN model with one LSTM layer, 2 dense layers, and Hyperbolic tangent activation function
...
Forecasting for the test set and plot the result (Bidirectional LSTM)

```{r}
batch_size_plot <- 3000
lookback_plot <- 1
step_plot <- 1
...
p <-
    ggplot(plot_data, aes(x = V1, y = V2)) + geom_line(colour = "red",
                                                        size = 1,
                                                        alpha = 4 )+labs(y= "Normalized real and predicted values on the interval [0,1]", x = "2 days ahead")+ggtitle("Real.Values (red) Vs Predicted.Values (blue)")
p <-
    p + geom_line(
        aes(x = V1, y = pred_out),
        colour = "blue",
...
```{r}
modelsvm <- ksvm(x,y,type = "eps-svr", epsilon = 0.1, kernel="anovadot")
...
Fuzzy clustering model (SBC) and c-mean optimization with cluster ratio 0.3
