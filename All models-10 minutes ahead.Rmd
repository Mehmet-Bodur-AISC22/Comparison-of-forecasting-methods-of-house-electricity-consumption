---
title: "all forecsting models-10 minutes ahead"
...
Forecasting 10 minutes ahead. Models are BRNN with LSTM layer, SVR WITH (ANOVA RBF kernel), and SBC with Fuzzy C-mean.
...
generator <- function(data,
    ...
    function() {
        ...
        for (j in 1:length(rows)) {
            ...
            targets[[j]] <- data[rows[[j]] + delay, "Total"]
        }
...
70 consecutive random rows of data is selected from the main data set(starting point is random).
after each time of code execution, a different part of the data set will be selected
average of the RMSE and MAE of 10 times run calculated.
...
sys = function(N,n){
    r = sample(1:(N-n), 1)
    seq(r, ((r + 70) -1), 1)     #selecting 70 minutes consecutive random data set
}
sys_sample<-raw_data[sys(nrow(raw_data), 70), ]

data <- data.matrix(sys_sample[1:70, -1])
...
data2[is.nan(data2)] <- 0 #if there is any NaN value in the data change it to 0

data2 = data2[, colSums(data2[1:60,] != 0) > 0] #if there is any column in training set with all zero values omit the column


#write.csv(sys_sample, 'data-firstrun-10minutes.csv')
```

data preparation for our BRNN model
30 assigned to training set and 30 to verification data
...
Data preparation for SVM and SBC models, also preparing real values of foretasted output to evaluate our models later, ninth column is the output 

```{r}
data.train <- data2[1 : 60, ]

data.fit <- data.train[, -ncol(data.train)] # just include input variables of training set
data.tst <- data2[61:70, -ncol(data2) ]
x <- data.train[, -ncol(data.train)]
y <- data.train[,  ncol(data.train)] 

real.val.SVM <- matrix(data2[61:70, "Total"], ncol = 1)
real.val.SBC <- matrix(data2[61:70, "Total"], ncol = 1)
real.val.ANN <- matrix(data2[61:70, "Total"], ncol = 1)

range.data <- matrix(apply(data2, 2, range), nrow = 2)
```
...
callbacks = callback_early_stopping(monitor = "val_loss", min_delta = 0, patience = 30, verbose = 0, mode = "auto", baseline = NULL, restore_best_weights = TRUE) # preserving our current model
...
dim(inputdata) <- c(batch_size_plot, lookback_plot, ncol(data2))
...
Building the model SVR with ANOVA RBF kernel with epsilon = 0.01 with the default parameters

```{r}
modelsvm <- ksvm(x,y,type = "eps-svr", epsilon=0.01 ,kernel="anovadot",scaled = TRUE, kpar=list(sigma=1,degree = 1)) 
